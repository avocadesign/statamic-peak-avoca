...
- `display_settings`: Display settings. A switch in the editor that shows the fields depending on it. It saves nothing, so write the fields it reveals directly. Help: Change how this block looks: its layout, colour scheme and spacing.
- `column_widths`: Column layout. Choice (`select`). Options: `even` Even, `narrow-left` Left col narrow, `narrow-right` Right col narrow, `edge-center` Edge to center, `stacked` Text above media, `no-gap` No gap. Default `even`. Only when Display settings is on.
